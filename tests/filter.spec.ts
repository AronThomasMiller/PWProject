...
[{ isAsc: true }, { isAsc: false }].forEach(({ isAsc }) => {
  test(
    `Verify user can perform sorting by name (${isAsc ? "asc" : "desc"})`,
    { tag: "@regression" },
    async ({ homePage }) => {
      ...
      await test.step(`Sort products by name in ${
        isAsc ? "ascending" : "descending"
      } order`, async () => {
        await homePage.productsFilterFragment.sortByName(isAsc);
        await homePage.page.waitForLoadState("networkidle");
        await homePage.productsFilterFragment.expectProductNamesAreSorted(
          isAsc
        );
      });
    }
...
[{ isAsc: true }, { isAsc: false }].forEach(({ isAsc }) => {
  test(
    `Verify user can perform sorting by price (${isAsc ? "asc" : "desc"})`,
    { tag: "@regression" },
    async ({ homePage }) => {
      await test.step("Navigate to home page", async () => {
        await homePage.navigate();
      ...
      await test.step(`Sort products by price in ${
        isAsc ? "ascending" : "descending"
      } order`, async () => {
        await homePage.productsFilterFragment.sortByPrice(isAsc);
        await homePage.page.waitForLoadState("networkidle");
        await homePage.productsFilterFragment.expectProductPricesAreSorted(
          isAsc
        );
      });
    }
